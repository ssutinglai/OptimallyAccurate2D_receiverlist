subroutine create_color_image(image_data_2D,NX,NY,it,nSource,ix_rec,iy_rec,nrec, &
     NPOINTS_PML,USE_PML_XMIN,USE_PML_XMAX,USE_PML_YMIN,USE_PML_YMAX,field_number,lmargin)
  implicit none
...
integer NX,NY,it,field_number,NPOINTS_PML,nrec,lmargin
  ...
  integer :: ix,iy,irec,XSOURCE,JSOURCE,nSource,iSOURCE
  integer :: times,izSourceStart,iSourceInterval,iSourceStart
  integer, allocatable :: iisx(:), iisz(:)
!  integer :: lmargin(:)
  ...
  allocate(iisx(nSource),iisz(nSource))
  ...
  do iSource = 1, nSource
     ...
     if((iSource.eq.4).or.(iSource.le.4)) then
       iisx(iSource)=(iSourceStart-1)*times+1+iSourceInterval*times*(iSource-1)+lmargin
       iisz(iSource)=(izSourceStart-1)*times+1-iSourceInterval*times*(iSource-1)+lmargin

     elseif(iSource.ge.4)then
       iisx(iSource)=(iSourceStart-1)*times+1+iSourceInterval*times*(iSource-1)+lmargin
       iisz(iSource)=(izSourceStart-1)*times+1-iSourceInterval*times*(nSource-iSource)+lmargin
     endif
  ...
  do iy=1,NY,1
    ...
    do iSource=1,nSource
        ...
        XSOURCE=iisx(iSource)
        JSOURCE=iisz(iSource)
!        write(*,*)XSOURCE,JSOURCE

        !       draw an orange cross to represent the source
        if((ix >= XSOURCE - width_cross .and. ix <= XSOURCE + width_cross .and. &
             iy >= JSOURCE - thickness_cross .and. iy <= JSOURCE + thickness_cross) .or. &
             (ix >= XSOURCE - thickness_cross .and. ix <= XSOURCE + thickness_cross .and. &
